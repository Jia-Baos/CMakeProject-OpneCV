...
cv::Mat PeriodTranslation_Mode(const cv::Mat& image, const cv::Mat& template1)
{
	...
	// 首次计算相似度，判断如何进行裁剪
	ComputeParameter(matTmp, template1, similarityTmp, locationTmp);

	if (similarityTmp[0] >= 0.6)
	{
	...
	while (similarityTmp[0] < 0.6)
	{
		std::cout << "iterating..." << std::endl;
		matTmp_croped = PeriodTranslation_GRAY(matTmp_croped, 100);
		similarityTmp.clear();
		locationTmp.clear();
		ComputeParameter(matTmp_croped, template1, similarityTmp, locationTmp);
	}
