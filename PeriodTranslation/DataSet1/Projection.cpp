...
bool GetVerProjRegions(const cv::Mat& image, std::vector<int>& regionIndex)
{
	...
	for (int col = 0; col < width; ++col)
	{
		...
		// 进入标签区域
		else if (inRegion && projArray[col] < 3)
		{
			inRegion = false;
			endIndex = col;
	...
	// 考虑到最后一个区域可能起始于边缘
	if (regionIndex.size() % 2 == 1)
	{
		regionIndex.push_back(height);
	}
...
bool GetHorProjRegions(const cv::Mat& image, std::vector<int>& regionIndex)
{
	...
	for (int row = 0; row < height; ++row)
	{
		...
		// 进入标签区域
		else if (inRegion && projArray[row] < 3)
		{
			inRegion = false;
			endIndex = row;
			regionIndex.push_back(endIndex);
